...
## Do Book Genre's and Book Formats effect Reading time?

This dynamic report will explore three variables, reading minutes, book format and book genre. From observations in Auckland Domain, I will firstly be exploring whether more people reading physical books or digital ones (e-books, audio books). Along with this, this report aims to look at genre popularity and how this correlates with reading time. You need to change the contents of this Rmd! Replace the R code and markdown give below in this template document.
...
This plot explores how often Physical books and Digital Books were seen in Auckland Domain. Here we also explore the average time spent reading across the observations.
```{r}
...
num_observation <- latest_data$reading_minutes%>%
  length()
max_reading <- latest_data$book_format%>%
  max()
...
popular_genre <- max(latest_data$book_genre)

least_genre <- min(latest_data$book_genre)

num_observation <- length(latest_data$book_genre)

paste("From the people observed at Auckland Domain, the most popular genre was", popular_genre, ". Whilst the least popular genre was",least_genre, "out of the", num_observation, "people that observed reading over a 7 day period.")
```